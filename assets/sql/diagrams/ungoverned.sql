...
SELECT * FROM (VALUES
  ...
  ('induced_into', '|categoryValueRef| = |induction|', 'diagrams/category_members', 'categoryValueRef',
              '⛔ AN INDUCTION DROPPED, WHICH IS THE STATE THIS CORPUS WAS IN. One operation here draws from `labour` and induces into `capability`; a lane set can hold it in one place and it was held in the draw, so the second incidence was absent from every emitted document while `|lane| = |layer|` passed'),
  ('diagrams', '|BPMNDiagram| = |filing|', 'diagrams/pools', 'BPMNDiagram',
              '⭐⭐⭐ THE SVG''S PROPER PLACE IN THE DOCUMENT, EMPTY UNTIL NOW. `bpmndi:BPMNDiagram` is in `tDefinitions`''s own sequence and this emitter never wrote one, so the SVG stage INVENTED its coordinates and the document, opened in any tool, would lay itself out differently. Two pictures of one model with nothing tying them'),
