...
CREATE VIEW algebra.roster AS -- the set-algebraic laws this tree's relations claim to obey.
SELECT * FROM (VALUES
...
;
CREATE VIEW composition.composer_regimes AS -- asrt:regime at the root of an asrt:composition, one row per declaration in document order.
...
;
CREATE VIEW composition.notations AS -- pm:processModulus/pm:notation: uri -> filing, with the party that asserted the identity.
SELECT fi.notation, fi.filing, fi.asserted_by, fi.absent
FROM pm.filing_identity fi
;
CREATE VIEW composition.part_references AS -- asrt:Composition/asrt:Fusion/asrt:Part, keyed by pm:ForeignId (notation + id).
...
FROM pm.part p
;
CREATE VIEW composition.regimes AS -- pm:Regime, one row per declaration in document order.
SELECT r.filing, r.seq, r.id, r.jurisdiction,
       r.framework_taxonomy, r.framework_value, r.framework_absent,
       r.chart_taxonomy, r.chart_value, r.chart_absent
FROM pm.regime r
;
CREATE VIEW eliminations.filed AS -- asrt:Fusion/asrt:eliminations/asrt:elimination, per composed layer and quantity.
SELECT e.composition, e.composed_layer, e.quantity,
       e.low, e.mode, e.high, e.unit,
       e.absent, e.derivation, e.reason, e.claim_seq
FROM pm.elimination e
;
CREATE VIEW eliminations.searched AS -- asrt:Fusion/asrt:eliminations/asrt:absent, one row per composed layer asked.
SELECT es.composition, es.composed_layer, es.absent AS answer, es.note
FROM pm.elimination_search es
;
CREATE VIEW entries.couplings AS -- pm:Stack/pm:couplings/pm:coupling, each carrying its pm:observed.
SELECT c.filing, c.from_layer, c.to_layer,
       c.low, c.mode, c.high, c.unit, c.strength_absent, c.observation
FROM pm.coupling c
;
CREATE VIEW entries.draws AS -- pm:Operation/pm:Draw.
SELECT d.filing, d.operation, d.layer,
       d.low, d.mode, d.high, d.unit, d.absent
FROM pm.draw d
;
CREATE VIEW entries.holders AS -- pm:Remainder/pm:holder; kind is pm:HolderKind.
...
FROM pm.holder h
;
CREATE VIEW entries.inductions AS -- pm:Operation/pm:Induction, carrying pm:decidedBy.
SELECT n.filing, n.operation, n.layer,
       n.low, n.mode, n.high, n.unit, n.absent, n.decider
FROM pm.induction n
;
CREATE VIEW entries.operations AS -- pm:Operation, keyed (filing, label), with the notation position or the reason there is none.
SELECT o.filing, o.label, o.foreign_notation, o.foreign_id, o.foreign_absent::text AS foreign_absent
FROM pm.operation o
;
CREATE VIEW entries.slacks AS -- pm:Layer/pm:timeSlack with pm:Nameplate/pm:capacitySlack and pm:inventorySlack; the element names are the kinds.
...
FROM pm.slack s
;
CREATE VIEW epistemics.claims AS -- pm:Claim, with its required pm:narrowsWhen and pm:boundOrigin, joined on the claim.
...
;
CREATE VIEW epistemics.coupling_searches AS -- pm:Stack/pm:couplings/pm:absent, one row per filing asked.
SELECT cs.filing, cs.absent AS answer, cs.note
FROM pm.coupling_search cs
;
CREATE VIEW epistemics.filed_absences AS -- pm:Absence and pm:ClaimAbsence, every one, with pm:note, pm:asOf and pm:provenance.
...
FROM pm.absence a
;
CREATE VIEW epistemics.filed_derivations AS -- pm:Derivation and its restrictions, every one, with pm:note, pm:asOf and pm:provenance.
...
;
CREATE VIEW epistemics.scopes AS -- pm:Stack/pm:scope, with its pm:basis.
SELECT ss.filing, ss.extent, ss.basis, ss.absent
FROM pm.stack_scope ss
;
CREATE VIEW layers.denied_remainders AS -- pm:Layer/pm:remainder taking the pm:absent branch of pm:StatedRemainder.
...
;
CREATE VIEW layers.filed_remainders AS -- pm:Layer/pm:remainder taking the pm:claim branch of pm:StatedRemainder.
SELECT l.filing, l.layer,
...
;
CREATE VIEW layers.patience AS -- pm:Demand/pm:patience, beside the demand it qualifies.
SELECT l.filing, l.layer,
...
FROM pm.layer l
;
CREATE VIEW layers.summed_quantities AS -- pm:Layer/pm:Demand, pm:Nameplate/pm:amount and pm:Jagged/pm:draw, one row per quantity.
...
FROM pm.nameplate n
;
CREATE VIEW layers.windows AS -- pm:Nameplate/pm:Divisibility/pm:window, beside the amount unit that decides if it is answerable.
...
FROM pm.nameplate n
;
CREATE VIEW scope.every_filing AS -- from pm.filing: both evidence values, the typed reason a document gives neither, and an assertion's provenance.
...
FROM pm.filing f
;
CREATE VIEW epistemics.absences AS -- every pm:absent/reason in the schema, from every element that admits one.
...
;
CREATE VIEW arithmetic.roster AS -- the arithmetic the schemas' prose owes, against the unit rules that exist to make it mean anything.
SELECT * FROM (VALUES
...
;
CREATE VIEW composition.fusions AS -- asrt:Fusion: the composed layer it names, and asrt:observed.
SELECT f.composition AS filing, f.composed_layer AS layer, f.observed
FROM pm.fusion f
;
CREATE VIEW composition.parts AS -- pm.part joined through pm.filing_identity to pm.layer.
...
;
CREATE VIEW composition.unresolved_parts AS -- composition/part_references.sqlc less composition/parts.sqlc, per fusion and reference.
...
;
CREATE VIEW folds.fusion_parts AS -- composition/part_references.sqlc folded to one row per fusion it names.
...
;
CREATE VIEW composition.derived_quantities AS -- composition/derived_frontier.sqlc summed at the nodes stating the figure, less eliminations/filed.sqlc at the root and each derived node passed.
WITH
root AS (
...
;
CREATE VIEW composition.resolved_quantities AS -- layers/summed_quantities.sqlc where no derivation is filed, beside composition/derived_quantities.sqlc where one is.
...
;
CREATE VIEW layers.nameplate AS -- pm:Layer/pm:Nameplate, its Divisibility and its window, with the amount from
...
;
CREATE VIEW layers.drawn AS -- pm:Jagged/pm:draw from composition/resolved_quantities.sqlc, against layers/nameplate.sqlc and entries/slacks.sqlc's capacity slack.
...
;
CREATE VIEW entries.unserved_holders AS -- pm:HolderKind values `customer` and `unrealised`.
SELECT h.*
FROM (
    SELECT * FROM entries.holders
) h
...
;
CREATE VIEW entries.unserved_totals AS -- entries/unserved_holders.sqlc folded to one row per layer.
...
;
CREATE VIEW layers.demand AS -- composition/resolved_quantities.sqlc's pm:Layer/pm:Demand, where it has a figure.
...
WHERE q.quantity = 'demand'
  AND q.low IS NOT NULL
;
CREATE VIEW composition.fusion_quantities AS -- composition/fusions.sqlc joined to layers/summed_quantities.sqlc on the composed layer.
...
;
CREATE VIEW composition.unsized_conversions AS -- asrt:Part/asrt:factor taking its pm:absent or pm:derivation branch, as a suspension of the composed sum.
SELECT p.composition, p.composed_layer,
       NULL::pm.summed_quantity AS quantity,
...
;
CREATE VIEW composition.filed_grounds AS -- the five grounds decided by what is filed, one row per ground, carrying the quantity it lifts.
...
;
CREATE VIEW composition.unsettled AS -- composition/parts.sqlc restricted to the parts whose factor has width or no figure.
...
;
CREATE VIEW composition.remainder_frontier AS -- composition/parts.sqlc walked while composition/unsettled.sqlc holds, carrying the product.
WITH RECURSIVE
resolved AS (
    SELECT * FROM composition.parts
),
...
FROM frontier f
;
CREATE VIEW composition.passed_nodes AS -- composition/remainder_frontier.sqlc restricted to the nodes composition/unsettled.sqlc names.
SELECT w.*
FROM      (
...
;
CREATE VIEW composition.suspended_remainders AS -- composition/filed_grounds.sqlc restricted to the two quantities r is built from, at the layer or a node its walk passes;
...
WHERE q.quantity IN ('demand', 'nameplate')
  AND q.low IS NULL
;
CREATE VIEW composition.figureless_remainders AS -- composition/unsettled.sqlc within composition/suspended_remainders.sqlc.
...
;
CREATE VIEW composition.converted AS -- asrt:Part/asrt:factor applied to each quantity the part layer states.
...
;
CREATE VIEW folds.part_sums AS -- composition/converted.sqlc folded to one row per composed layer and quantity.
...
;
CREATE VIEW eliminations.paired AS -- eliminations/filed.sqlc against folds/part_sums.sqlc.
...
;
CREATE VIEW composition.owed_remainder AS -- composition/fusions.sqlc less composition/suspended_remainders.sqlc.
...
;
CREATE VIEW layers.figures AS -- layers/demand.sqlc and layers/nameplate.sqlc on the layer, each side where it has a figure.
SELECT filing, layer,
...
;
CREATE VIEW layers.differenced_remainder AS -- layers/figures.sqlc where both figures are present, beside the sign and absorber of
...
;
CREATE VIEW composition.settled_remainders AS -- composition/remainder_frontier.sqlc less the nodes composition/unsettled.sqlc names,
...
;
CREATE VIEW composition.fused_remainders AS -- composition/settled_remainders.sqlc summed over the settled frontier, less eliminations/paired.sqlc at their own corners.
...
;
CREATE VIEW layers.remainder AS 
...
;
CREATE VIEW layers.exposure_scope AS -- layers/remainder.sqlc where exposure > 0, classified by layers/absorption.sqlc.
...
;
CREATE VIEW layers.unabsorbed_exposure AS -- layers/exposure_scope.sqlc, restricted to the standing that licenses a conclusion.
SELECT s.*
FROM (
...
;
CREATE VIEW layers.filed_against_derived AS -- layers/filed_remainders.sqlc against layers/remainder.sqlc, on the layer they share.
SELECT l.filing, l.layer,
       l.qty_low, l.qty_mode, l.qty_high, l.qty_unit, l.qty_absent,
...
;
CREATE VIEW composition.derived_fusions AS -- composition/fusion_quantities.sqlc filed as a derivation.
...
;
CREATE VIEW composition.suspension_grounds AS -- the grounds that lift the sum rule: those read off the filing, and a derived part that cannot be computed.
SELECT * FROM composition.filed_grounds
UNION ALL
...
;
CREATE VIEW composition.suspended_fusions AS -- composition/suspension_grounds.sqlc projected onto the fusion it suspends.
SELECT DISTINCT g.composition, g.composed_layer, g.quantity
FROM (
    SELECT * FROM composition.suspension_grounds
) g
;
CREATE VIEW composition.suspended_quantities AS -- composition/suspended_fusions.sqlc expanded onto pm.summed_quantity.
...
;
CREATE VIEW composition.owed_equality AS -- composition/fusions.sqlc for every quantity, minus the suspensions that lift each sum and the
...
;
CREATE VIEW layers.absorber AS -- pm:Remainder/absorber, resolved through pm.buffer_term.
SELECT l.filing, l.layer,
...
;
CREATE VIEW entries.absorbing_slack AS -- layers/absorber.sqlc joined to entries/slacks.sqlc on the buffer the layer actually names.
...
;
CREATE VIEW entries.served_holders AS -- pm:HolderKind values `booked`, `counterparty` and `people`.
SELECT h.*
FROM (
    SELECT * FROM entries.holders
) h
...
;
CREATE VIEW folds.served_totals AS -- entries/served_holders.sqlc folded to one row per layer.
SELECT h.filing, h.layer,
       count(*)                                        AS holders,
...
GROUP BY h.filing, h.layer
;
CREATE VIEW layers.pressed AS -- pm:Remainder/sign in {interference, transition}.
SELECT r.*
FROM (
    SELECT * FROM layers.remainder
) r
...
;
CREATE VIEW entries.holder_totals AS -- entries/holders.sqlc folded to one row per layer.
SELECT h.filing, h.layer,
       count(*)                                     AS holders,
       count(*) FILTER (WHERE h.share_mode IS NULL)  AS unstated,
       count(*) FILTER (WHERE h.share_derivation IS NOT NULL) AS derived,
...
GROUP BY h.filing, h.layer
;
CREATE VIEW entries.derived_time_slacks AS -- pm:Layer/pm:timeSlack filed as a clearance derivation, beside pm:Divisibility/pm:window.
...
;
CREATE VIEW layers.lumpy AS -- layers/figures.sqlc where the nameplate is lumpy, the demand beside it where stated.
SELECT f.filing, f.layer,
...
WHERE f.lumpy
;
CREATE VIEW arithmetic.all AS -- arithmetic/roster.sqlc joined to each site's own population.
...
;
CREATE VIEW layers.quantities AS -- pm:Layer's own quantities: demand, nameplate and the three buffer slacks, keyed by element.
...
;
CREATE VIEW composition.part_quantities AS -- composition/parts.sqlc with layers/quantities.sqlc at the part's layer and at the composed layer.
...
;
CREATE VIEW composition.carriable AS -- composition/part_quantities.sqlc for a fusion that files one part, where the factor has a figure.
...
WHERE q.factor_state IN ('omitted', 'stated')
  AND f.parts = 1
;
CREATE VIEW composition.carried AS -- composition/carriable.sqlc less the fusions that eliminate or owe no sum.
SELECT c.*
FROM      (
...
;
CREATE VIEW composition.composed_quantum AS -- composition/parts.sqlc against layers/lumpy.sqlc, converted by asrt:factor and folded by gcd, over every part in composition/part_references.sqlc.
...
LEFT JOIN fold       f ON f.composition = s.composition
                      ...
                      AND f.i = s.sized
;
CREATE VIEW composition.fused AS -- folds/part_sums.sqlc against the composed layer's own figure, less eliminations/paired.sqlc at its corners.
SELECT x.composition, x.composed_layer, x.quantity,
...
;
CREATE VIEW checks.roster AS -- the conformance rules stated in the schemas' prose and gated by no grammar.
SELECT * FROM (VALUES
...
;
CREATE VIEW epistemics.point_claims AS -- pm:Claim where low = high, at every element that carries one.
SELECT c.*
FROM (
    SELECT * FROM epistemics.claims
) c
WHERE c.is_a_point
;
CREATE VIEW units.conversions AS -- asrt:Part/asrt:factor at the nameplate, as part-layer-unit to composed-layer-unit.
SELECT DISTINCT
...
;
CREATE VIEW units.with_a_period AS -- pm:Claim/pm:denominator/pm:period, at pm:Nameplate/amount.
...
;
CREATE VIEW identities.roster AS -- pm:Identity against every *Derivation restriction in the two schemas, per position and arm.
SELECT * FROM (VALUES
...
;
CREATE VIEW composition.descent AS -- asrt:Fusion/asrt:Part followed transitively through pm.filing_identity.
WITH RECURSIVE
resolved AS (
    SELECT * FROM composition.parts
),
...
FROM walk
;
CREATE VIEW composition.jagged_layers AS -- composition/parts.sqlc self-joined on the fusion, against the reflexive closure of
...
;
CREATE VIEW layers.every_layer AS -- pm:Stack/pm:layer, keyed and nothing more.
SELECT l.filing, l.layer
FROM pm.layer l
;
CREATE VIEW composition.part_regimes AS -- composition/parts.sqlc's regime handle resolved into composition/composer_regimes.sqlc, against
...
;
CREATE VIEW composition.citations AS -- asrt:composition/asrt:citation, one row each.
SELECT c.composition, c.seq, c.taxonomy, c.instrument, c.clause, c.version
FROM pm.composition_citation c
;
CREATE VIEW checks.unresolved_part AS -- asrt:Part/pm:ForeignId against pm.filing_identity and pm.layer.
...
;
CREATE VIEW checks.jagged_layer AS -- asrt:Fusion/asrt:Part against itself; conformance rule "a fusion's parts partition what they compose".
SELECT r.rule, p.filing, p.layer, p.violates, p.detail
FROM      (
    SELECT * FROM checks.roster
) r
...
;
CREATE VIEW checks.all AS -- the conformance rules XSD 1.0 cannot reach, one file each; checks/roster.sqlc is the list.
...
;
CREATE VIEW folds.rule_subjects AS -- checks/roster.sqlc against checks/all.sqlc, one row per rule.
...
;
CREATE VIEW rank.graph_edges AS -- composition/parts.sqlc and units/conversions.sqlc, each labelled with the graph it is an edge of.
...
;
CREATE VIEW rank.compose_edges AS -- public.compose_edge, generated by examples/compositions/main.rs from assets/sqlc/.
SELECT e.parent, e.child, e.splices, e.inner_joins
FROM public.compose_edge e
;
CREATE VIEW rank.graph_measures AS -- rank/graph_edges.sqlc, symmetrised and walked for components, counted at both scopes.
WITH RECURSIVE
edges AS (
...
;
CREATE VIEW rank.compose_measures AS -- rank/compose_edges.sqlc, symmetrised and walked for components, counted whole and by directory.
WITH RECURSIVE
edge AS (
...
GROUP BY  n.scope
;
CREATE VIEW rank.composition_kernel AS -- asrt:Fusion/asrt:Part folded onto its fusion: the fibre size, and the block dimension it fixes.
...
GROUP BY p.composition, p.composed_layer
;
CREATE VIEW rank.composition_image AS -- composition/fusions.sqlc against the fusions that are a row of F Phi.
...
GROUP BY f.filing
;
CREATE VIEW checks.fit_axes AS -- the fit axis each rule on checks/roster.sqlc reads, declared.
SELECT * FROM (VALUES
...
;
CREATE VIEW checks.fit_domain AS -- the declared standing of each rule on checks/fit_axes.sqlc in each cell of its axis.
SELECT * FROM (VALUES
...
;
CREATE VIEW reports.fit_coverage AS -- checks/fit_domain.sqlc against checks/fit_cells.sqlc.
...
;
CREATE VIEW diagrams.domain_objects AS -- the pm.* tables against BPMN 2.0's element vocabulary, as data an emitter reads.
SELECT * FROM (VALUES
...
;
CREATE VIEW eliminations.between AS -- asrt:Fusion/asrt:eliminations/asrt:elimination/asrt:between, one row each.
...
;
CREATE VIEW eliminations.references AS -- asrt:Elimination/asrt:between, typed asrt:FiledLayer, whose filing is a pm:ForeignId.
SELECT b.composition, b.composed_layer, b.quantity, b.seq,
...
;
CREATE VIEW eliminations.resolved AS -- eliminations/references.sqlc joined through pm.filing_identity to pm.layer.
SELECT b.composition, b.composed_layer, b.quantity, b.seq,
...
;
CREATE VIEW diagrams.categories AS -- pm:Operation/pm:Induction projected to the layers it reaches; BPMN 2.0 tCategoryValue.
WITH
...
;
CREATE VIEW diagrams.citations AS -- asrt:Composition/asrt:citation, flattened to the one string a documentation element can hold.
WITH
...
;
CREATE VIEW diagrams.dependences AS -- pm:Stack/pm:Coupling projected to its two ends; BPMN 2.0 tAssociation sourceRef/targetRef.
WITH
...
;
CREATE VIEW diagrams.calls AS -- composition/parts.sqlc projected to F alone, with Phi dropped; one call activity per part.
WITH
...
;
CREATE VIEW diagrams.descents AS -- asrt:Fusion/asrt:Part crossing a document; BPMN 2.0 tRelationship source/target.
WITH
notations AS NOT MATERIALIZED (
    -- pm:processModulus/pm:notation: uri -> filing, with the party that asserted the identity.
SELECT fi.notation, fi.filing, fi.asserted_by, fi.absent
...
;
CREATE VIEW epistemics.documents AS -- The five top-level declarations: pm:processModulus and asrt:composition/dependence/coverage/run.
...
;
CREATE VIEW diagrams.pools AS -- epistemics/documents.sqlc projected to the filing alone; one pool per document.
WITH
notations AS NOT MATERIALIZED (
    -- pm:processModulus/pm:notation: uri -> filing, with the party that asserted the identity.
SELECT fi.notation, fi.filing, fi.asserted_by, fi.absent
...
;
CREATE VIEW diagrams.scopes AS -- pm:Stack/pm:StatedScope; carried as the laneSet's own documentation beside the coupling search.
WITH
...
;
CREATE VIEW diagrams.searches AS -- pm:Stack/pm:couplings/pm:absent; carried as the laneSet's own documentation.
WITH
...
;
CREATE VIEW entries.notation_references AS -- pm:Operation/pm:notationPosition, the stated arm: a notation plus an id.
WITH
...
;
CREATE VIEW diagrams.foreign_calls AS -- diagrams/calls.sqlc pinned to the parts that leave their own document.
...
WHERE NOT c.is_local
;
CREATE VIEW diagrams.lane_membership AS -- entries/draws.sqlc and diagrams/foreign_calls.sqlc, each projected onto the lane it belongs to.
...
;
CREATE VIEW diagrams.legends AS -- the notes a correct reading requires; BPMN 2.0 tTextAnnotation, drawn and unattached.
WITH
notations AS NOT MATERIALIZED (
    -- pm:processModulus/pm:notation: uri -> filing, with the party that asserted the identity.
SELECT fi.notation, fi.filing, fi.asserted_by, fi.absent
...
FROM ( SELECT * FROM diagrams.scopes ) s
UNION ALL
SELECT s.filing, 'search', 'diagrams/searches.sqlc'
FROM ( SELECT * FROM diagrams.searches ) s
UNION ALL
...
FROM ( SELECT * FROM diagrams.categories ) c
UNION ALL
...
;
CREATE VIEW diagrams.expected AS -- diagrams/roster.sqlc's model_side relations, each counted.
WITH
...
SELECT 'pools'   AS slug, count(*) AS expected FROM ( SELECT * FROM epistemics.documents )  x
UNION ALL
SELECT 'lanes',   count(*) FROM ( SELECT * FROM layers.every_layer )    x
UNION ALL
SELECT 'tasks',   count(*) FROM ( SELECT * FROM entries.operations )    x
UNION ALL
SELECT 'calls',   count(*) FROM ( SELECT * FROM diagrams.foreign_calls ) x
UNION ALL
...
UNION ALL
SELECT 'categories', count(DISTINCT x.filing) FROM ( SELECT * FROM diagrams.categories ) x
UNION ALL
SELECT 'category_values', count(*) FROM ( SELECT * FROM diagrams.categories ) x
UNION ALL
SELECT 'groups',     count(*) FROM ( SELECT * FROM diagrams.categories ) x
UNION ALL
...
UNION ALL
SELECT 'diagrams', count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
SELECT 'planes',   count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
...
UNION ALL
SELECT 'legends', count(*) FROM ( SELECT * FROM diagrams.legends ) x
UNION ALL
SELECT 'descents', count(*) FROM ( SELECT * FROM diagrams.descents ) x
UNION ALL
SELECT 'attributions', count(*) FROM ( SELECT * FROM eliminations.resolved ) x
UNION ALL
SELECT 'citations', count(*) FROM ( SELECT * FROM diagrams.citations ) x
UNION ALL
SELECT 'scopes', count(*) FROM ( SELECT * FROM diagrams.scopes ) x
UNION ALL
SELECT 'dependences', count(*) FROM ( SELECT * FROM diagrams.dependences ) x
UNION ALL
SELECT 'namespaces', count(*) FROM ( SELECT * FROM composition.notations ) x
UNION ALL
...
UNION ALL
SELECT 'in_a_lane', count(*) FROM ( SELECT * FROM diagrams.lane_membership ) x
UNION ALL
SELECT 'definitions',   count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
SELECT 'collaboration', count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
SELECT 'process',       count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
SELECT 'lane_set',      count(*) FROM ( SELECT * FROM diagrams.pools ) x
UNION ALL
...
FROM ( SELECT * FROM entries.notation_references ) n
UNION ALL
...
;
CREATE VIEW diagrams.roster AS -- the cardinality identities a BPMN emission owes, against the relation supplying each expected count.
SELECT * FROM (VALUES
...
;
CREATE VIEW epistemics.absence_columns AS -- information_schema.columns, restricted to schema pm and type pm.absence_reason.
...
;
CREATE VIEW epistemics.derivation_columns AS -- information_schema.columns, restricted to schema pm and type pm.identity.
SELECT c.table_name::text  AS table_name,
       c.column_name::text AS column_name
FROM information_schema.columns c
WHERE c.table_schema = 'pm'
...
;
CREATE VIEW folds.absence_subjects AS -- epistemics/absence_questions.sqlc against epistemics/absence_columns.sqlc, one row per column.
...
;
CREATE VIEW folds.derivation_subjects AS -- identities/roster.sqlc's columns against epistemics/derivation_columns.sqlc, one row per column.
...
;
CREATE VIEW folds.diagram_law_subjects AS -- diagrams/roster.sqlc against diagrams/expected.sqlc, one row per diagram law.
...
;
CREATE VIEW folds.object_subjects AS -- diagrams/domain_objects.sqlc against diagrams/catalogue.sqlc, one row per table.
...
;
CREATE VIEW folds.site_subjects AS -- arithmetic/roster.sqlc against arithmetic/all.sqlc, one row per site.
...
;
CREATE VIEW layers.decomposed AS -- layers/lumpy.sqlc, split as r = m*q - (demand mod q) at the crossed pairs.
...
;
CREATE VIEW entries.spillovers AS -- pm:couplings/pm:coupling, projected onto every other filing holding both of its ends.
...
;
CREATE VIEW layers.remainder_scope AS -- layers/remainder.sqlc against pm:Stack/pm:scope, pm:couplings/pm:absent and entries/spillovers.sqlc.
SELECT r.filing, r.layer,
       sc.extent,
       cs.answer AS search,
...
;
CREATE VIEW epistemics.searches AS -- pm:Stack/pm:couplings and asrt:Fusion/asrt:eliminations, each with its pm:absent.
...
;
CREATE VIEW composition.sibling_parts AS -- composition/parts.sqlc crossed with itself on the composed layer and the part filing.
...
;
CREATE VIEW eliminations.derived AS -- composition/parts.sqlc crossed with itself on the part each side names, with that part's own figures.
...
;
CREATE VIEW epistemics.widths AS -- pm:Claim/pm:narrowsWhen, wherever a claim appears.
...
;
CREATE VIEW folds.law_subjects AS -- algebra/roster.sqlc against algebra/all.sqlc, one row per law.
...
GROUP BY l.filing, l.layer
;
CREATE VIEW reports.integrity AS -- folds/contract_subjects.sqlc crossed with the problems a contract can have, kept where one is found.
...
WHERE p.found
;
CREATE VIEW scope.corpus AS -- from pm.filing where evidence = 'observation' and the kind attests to a world.
