...
LEFT JOIN (
    ...
    FROM ( SELECT
             (SELECT count(*) FROM ( -- composition/parts.sqlc self-joined on the fusion, against the reflexive closure of
...
FROM pm.layer l
     ...
     AND r2.filing = r1.filing AND r2.layer = r1.layer
               ...
               WHERE NOT EXISTS (SELECT 1 FROM ( -- asrt:Fusion/asrt:Eliminations/asrt:elimination, per composed layer and quantity.
SELECT e.composition, e.composed_layer, e.quantity,
       e.low, e.mode, e.high, e.unit,
       e.absent, e.reason
FROM pm.elimination e
...
SELECT * FROM (VALUES
  ...
  ('denied_remainder_is_not_contradicted',
               ...
               (SELECT count(*) FROM ( SELECT r.rule FROM ( -- the conformance rules stated in the schemas' prose and gated by no grammar.
SELECT * FROM (VALUES
  ('fit_disagrees',                        'sign agrees with the range comparison'),
  ('shares_do_not_sum',                    'stated shares sum to the magnitude'),
  ('nobody_named_as_unserved',             'a supply with nowhere to put its excess names who went unserved'),
  ...
  ('denied_remainder_is_not_contradicted',
                                       ...
                                       EXCEPT
                                       SELECT c.rule FROM ( -- the conformance rules XSD 1.0 cannot reach, one file each; checks/roster.sqlc is the list.
...
SELECT * FROM (VALUES
  ...
  ('denied_remainder_is_not_contradicted',
                 ...
                 WHERE EXISTS (SELECT 1 FROM ( -- the conformance rules XSD 1.0 cannot reach, one file each; checks/roster.sqlc is the list.
-- pm:Remainder/sign against pm:Demand and pm:Nameplate; conformance rule "sign agrees".
SELECT r.rule, p.filing, p.layer, p.violates, p.detail
FROM      (
    -- the conformance rules stated in the schemas' prose and gated by no grammar.
...
WHERE r.slug = 'part_regime_disagrees'
        ...
        UNION ALL
        SELECT 'arithmetic',
               (SELECT count(DISTINCT b.site) FROM ( -- the arithmetic the schemas' prose owes, against the unit rules that exist to make it mean anything.
SELECT * FROM (VALUES
  ...
  ('fusion_sum',         'x_composed = F.Phi.x - e',     'parts x factors x elimination',   '(forbidden)')
               ...
               (SELECT count(*) FROM ( SELECT b.site FROM ( -- the arithmetic the schemas' prose owes, against the unit rules that exist to make it mean anything.
SELECT * FROM (VALUES
  ('remainder',          'r = n - d',                    'demand x nameplate',              NULL),
  ('shares_sum',         'sum of shares = |r|',          'holder shares x remainder',       NULL),
  ('shares_bounded',     'sum of shares <= S',           'holder shares x absorbing slack', 'slack_unit_mismatch'),
  ...
  ('fusion_sum',         'x_composed = F.Phi.x - e',     'parts x factors x elimination',   '(forbidden)')
                                       ...
                                       EXCEPT
                                       SELECT z.site FROM ( -- arithmetic/roster.sqlc joined to each site's own population.
...
SELECT * FROM (VALUES
  ...
  ('fusion_sum',         'x_composed = F.Phi.x - e',     'parts x factors x elimination',   '(forbidden)')
                 ...
                 WHERE EXISTS (SELECT 1 FROM ( -- arithmetic/roster.sqlc joined to each site's own population.
-- pm:Demand/pm:claim against pm:Nameplate/pm:amount, before layers/remainder.sqlc drops either.
SELECT a.site, p.filing, p.layer, p.verdict, p.detail
FROM      (
    -- the arithmetic the schemas' prose owes, against the unit rules that exist to make it mean anything.
