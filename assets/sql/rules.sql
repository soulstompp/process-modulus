...
\echo '   typed, the answer that leads is that NOTHING sets the edge -- the range is simply'
\echo '   where the measurements fell -- with `derived` close behind it, where the model'
\echo '   already states the author in a sibling element and had no way to point at it.'
...
\echo '   see it. The ingest aligned these two the day they were split out; nothing joined'
\echo '   them until now.'
...
\echo
\echo '⛔ And the roster against itself: a rule declared with no check behind it, or a check'
\echo '  nobody declared. An empty table is the correct outcome and the only one.'
