...
FROM (
    SELECT c.from_unit AS unit FROM ( -- asrt:Part/asrt:factor at the nameplate, as part-layer-unit to composed-layer-unit.
SELECT DISTINCT
       part.unit AS from_unit,
       comp.unit AS to_unit,
       p.factor_low, p.factor_mode, p.factor_high,
...
WHERE p.factor_low IS NOT NULL
    ...
    UNION
    SELECT c.to_unit        FROM ( -- asrt:Part/asrt:factor at the nameplate, as part-layer-unit to composed-layer-unit.
SELECT DISTINCT
       part.unit AS from_unit,
       comp.unit AS to_unit,
       p.factor_low, p.factor_mode, p.factor_high,
