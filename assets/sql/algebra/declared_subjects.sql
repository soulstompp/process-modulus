...
FROM ( -- the conformance rules stated in the schemas' prose and gated by no grammar.
SELECT * FROM (VALUES
...
FROM ( -- the arithmetic the schemas' prose owes, against the unit rules that exist to make it mean anything.
SELECT * FROM (VALUES
...
FROM ( -- the set-algebraic laws this tree's relations claim to obey.
SELECT * FROM (VALUES
...
FROM ( -- the pm.* tables against BPMN 2.0's element vocabulary, as data an emitter reads.
SELECT * FROM (VALUES
...
FROM ( -- the cardinality identities a BPMN emission owes, against the relation supplying each expected count.
SELECT * FROM (VALUES
